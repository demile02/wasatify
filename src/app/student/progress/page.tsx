import { Award, BookOpenCheck, CheckCircle2, Clock, Flame, MessageSquareText, Star } from 'lucide-react';
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Lihat capaian modul, XP, streak, waktu belajar, dan badge yang sudah kamu raih."
      />
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          description={`${progressData.completedModules.length} dari ${progressData.modules.length} modul selesai`}
          icon={Star}
        />
        ...
        <StatCard
          label="Waktu Belajar"
          value={formatStudyTime(progressData.studyMinutes)}
          description="Estimasi dari progress modul"
          icon={Clock}
          tone="mint"
        />
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1fr_0.72fr]">
        ...
        <SectionCard>
          <div className="flex items-center justify-between gap-3">
            ...
            <span className="text-xs font-semibold text-muted-foreground">{progressData.badges.length} badge</span>
          </div>
          ...
          {progressData.badges.length ? (
            <div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2">
              {progressData.badges.slice(0, 4).map((badge) => (
                <div key={`${badge.id}-${badge.earnedAt}`} className="rounded-2xl border border-border bg-white p-4 text-center">
                  <div className="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-2xl bg-cream text-gold">
                  ...
                  <p className="mt-2 text-xs font-bold text-primary">+{badge.xpReward} XP</p>
                </div>
              ))}
            </div>
          ) : (
